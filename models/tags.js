...
class Tags {
    ...
    static async create(tags, reading_id, user_id) {
        const tagsArray = tags.split('#').filter(tag => tag !== '').map(tag => [tag.trim()]);
        ...
        const tagIds = await Promise.all(tagsArray.map(async tag => await this.findIdByTagName(tag)));

        const readingTags = tagIds.map(tag_id => [reading_id, tag_id[0].id]);
        const insertReadingTags = await new Promise((resolve, reject) => {
            db.connection.query(queries.insertReadingTags, [readingTags], (err, results) => {
                if (err) reject(err);
                else resolve(results);
            });
        });
        ...
        const insertUserTags = await new Promise((resolve, reject) => {
            db.connection.query(queries.insertUserTags, [userTags], (err, results) => {
                if (err) reject(err);
                else resolve(results);
            });
        });
    ...
    static findSubscriptionTags(user_id) {
        let tags = new Promise((resolve, reject) => {
            db.connection.query(queries.selectSubscriptionTags, user_id, (err, results) => {
                if (err) reject(err);
                else resolve(results);
            });
        });
    ...
    static async deleteFromReading(reading_id, tags, user_id) {
        ...
        const deletedReadingTags = new Promise((resolve, reject) => {
            db.connection.query(queries.deleteReadingTags, [[readingTags]], (err, results) => {
                if (err) reject(err);
                else resolve(results);
            })
        })
        ...
        const deletedUserTags = new Promise((resolve, reject) => {
            db.connection.query(queries.deleteUserTags, [[userTags]], (err, results) => {
                if (err) reject(err);
                else resolve(results);
            });
        });
