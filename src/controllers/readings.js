...
exports.createReading = async (req, res, next) => {
  try {
    ...
    if (req.body.tags)
      await Tags.create(req.body.tags, reading_id, req.params.id);
...
exports.markFavorite = async (req, res, next) => {
  try {
    let favoritedReading = await Reading.markFavorite(
      req.params.id,
      req.params.user_id
    );
...
exports.deleteFavorite = async (req, res, next) => {
  try {
    let favoritedReading = await Reading.deleteFavorite(
      req.params.id,
      req.params.user_id
    );
    return res.status(200).json(favoritedReading);
...
exports.deleteReading = async (req, res, next) => {
  try {
    ...
    let deletedReading = await Reading.delete(
      req.params.id,
      req.params.reading_id
    );
...
exports.updateReading = async (req, res, next) => {
  try {
    ...
    let reading = await Reading.update(
      req.body.url,
      req.body.user_id,
      req.params.reading_id
    );
