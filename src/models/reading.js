...
class Reading {
  ...
  static async create(url, user_id) {
    ...
    const insertUserReading = await new Promise((resolve, reject) => {
      db.connection.query(
        queries.insertUserReading,
        [parseInt(user_id), reading_id],
        (err, results) => {
          if (err) reject(err);
          else resolve(results);
        }
  ...
  static findSubReadings(sub_id) {
    let subscriptionReadings = new Promise((resolve, reject) => {
      db.connection.query(
        queries.selectSubscriptionReadings,
        sub_id,
        function (err, results) {
          if (err) reject(err);
          else resolve(results);
  ...
  static findFavoriteReadings(id) {
    let favoriteReadings = new Promise((resolve, reject) => {
      db.connection.query(
        queries.selectFavoriteReadings,
        id,
        function (err, results) {
          if (err) reject(err);
          return resolve(results);
